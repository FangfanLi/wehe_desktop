...
import sys, os, ConfigParser, math, json, time, subprocess, commands, \
...
class RequestSet(object):
    ...
    def __init__(self, payload, c_s_pair, response, timestamp):
        ...
        if response is None:
            ...
            self.response_hash = hashlib.sha1(response.decode('hex')).hexdigest()
...
class ResponseSet(object):
    ...
    def __init__(self, request, response_list):
        ...
        self.request_hash = hashlib.sha1(request.decode('hex')).hexdigest()
