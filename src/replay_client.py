...
import sys, commands, socket, time, numpy, threading, select, pickle, Queue, urllib, urllib2
...
errorQ    = Queue.Queue()
# This is the return value for run()
replayResult = 'Finish'
...
class SideChannel(object):
    ...
    def identify(self, replayName, endOfTest, extraString='extraString', realIP = '127.0.0.1', size=10):
        ...
        self.send_object(';'.join([self.id, Configs().get('testID'), replayName, str(extraString), str(self.historyCount), str(endOfTest), realIP, '1.0']))
...
def run(configs = Configs(), pcapdir = None, cmpacNum = -1, caction = None, cspec = None, smpacNum = -1, saction = None, sspec = None, testID = '0', byExternal = False):
    initialSetup()
    ...
    # ASK for realIP
    # When on multiple interfaces, make sure using the same interface for asking IP and running replays
    ...
    # bucketNum is the number of data samples when calculating the throughputs on the client side
    bucketNum = 100
