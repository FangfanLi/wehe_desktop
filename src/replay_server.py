...
import pickle, atexit, re, urllib2, base64, glob, reverse_geocode, hashlib
...
class ClientObj(object):
    ...
    '''
    def __init__(self, incomingTime, realID, id, ip, replayName, testID, historyCount, extraString, connection, smpacNum, saction, sspec):
        self.id               = id
        self.replayName       = replayName
        ...
        if not os.path.exists( self.replayInfoFolder):
    ...
    def create_info_json(self, infoFile):
        ...
        return json.dump([self.incomingTime, self.realID, anonymizedIP, anonymizedIP, self.replayName, self.extraString, self.historyCount, self.testID,
                           self.exceptions, self.success, self.secondarySuccess, self.iperfRate, time.time()-self.startTime, self.clientTime, self.mobileStats],
                          open(infoFile, 'w'))
    
    def get_info(self):
        return map(str, [self.incomingTime, self.realID, self.id, self.ip, self.replayName, self.extraString, self.historyCount, self.testID, self.exceptions, self.success, self.secondarySuccess, self.iperfRate, time.time()-self.startTime, self.clientTime, self.mobileStats])
...
#             return
        ...
        #If we know who the client is:
            ...
            #The following exception handler is for checking header manipulations
                ...
                self.errorlog_q.put((id, 'Unknown packet from existing client', 'TCP', str(self.instance)))
                exceptionsReport = 'Unknown packet from existing client'

            if hashlib.sha1(new_data_4hash).hexdigest() not in self.LUT['tcp']:
            # if hash(new_data_4hash) not in self.LUT['tcp']:
        ...
        #If we DON'T know who the client is: (possibly because of IP flipping)
            ...
            if itsGET:
                ...
                connection.sendall('WhoTheFAreU?'+';'+clientIP)
                self.errorlog_q.put((id, 'Unknown packet from unknown client', 'TCP', str(self.instance)))
        ...
        clientO = self.all_clients[id].values()[0]
...
class SideChannel(object):
    ...
    def handle(self, connection, address):
        ...
        LOG_ACTION(logger, 'Server Load right now: CPU Usage {}% Memory Usage {}% Disk Usage {}% Upload Bandwidth Usage {}Mbps with {} active connections now ***'.format(cpuPercent, memPercent, diskPercent, upLoad, len(self.inProgress)))
        ...
        if memPercent > 95 or diskPercent > 95 or upLoad > 2000:
            ...
            dClient.exceptions = 'Server Overloaded with CPU Usage {}% Memory Usage {}% Upload Bandwidth Usage {}Mbps with {} active connections now *** '.format(cpuPercent, memPercent, upLoad, len(self.inProgress))
            self.errorlog_q.put('\t'.join(dClient.get_info()))
            return
        ...
        if good2go:
            LOG_ACTION(logger, 'Yay! Permission granted: {} - {} - {} - {}'.format(clientIP, realID, historyCount, testID), indent=2, action=False)
        ...
        if data[0] == 'WillSendMobileStats':
            ...
            mobileStats['locationInfo']['geoinfo'] = geoInfo
        ...
        if 'NoJitter' not in data:
            ...
            try:
                json.dump((xput, ts), open(xputFile, 'w'), 2 )
    ...
    def side_channel_callback(self, *args):
        ...
        if dClient.secondarySuccess:
            ...
            command = clean_pcap(dClient.dump.dump_name, dClient.ports, dClient.exceptions, hostList=dClient.hosts)
            tcpdumpends = time.time()
            cpuPercent, memPercent, diskPercent, upLoad = getSystemStat()
            # print '\r\n CLEANED PCAP FOR ', dClient.realID
            LOG_ACTION(logger,'Cleaned pcap for id: {}, historyCount: {}; CPU Usage {}% Memory Usage {}% Disk Usage {}% Upload Bandwidth Usage {}Mbps with {} active connections and {} clients now, spent {} seconds ***'.format(
                           dClient.realID, dClient.historyCount, cpuPercent, memPercent, diskPercent, upLoad,
                           len(self.inProgress), len(self.all_clients), tcpdumpends - tcpdumpstarts), indent=2, action=False)
    ...
    def replay_logger(self, replay_log):
        ...
        while True:
            ...
            if self.db is not None:
                print '\r\n Do NOT INSERT FOR NOW'
                # res = self.db.insertReplay(toWrite, self.instanceID)
                # if res != True:
                #     self.errorlog_q.put((toWrite[0],res))
            else:
